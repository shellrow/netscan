...
pub(crate) fn build_tcp_probe_packet(
    probe_setting: &ProbeSetting,
    probe_type: ProbeType,
    tcp_probe_kind: Option<TcpProbeKind>,
) -> Vec<u8> {
                    ...
                    ProbeType::TcpProbe => {
                        if let Some(probe_kind) = tcp_probe_kind {
                            ipv6_packet_builder.payload_length =
                                Some(probe_kind.ipv6_payload_length());
                        }
                    }
                    _ => {
    ...
    let mut tcp_packet_builder = TcpPacketBuilder::new(
        ...
        SocketAddr::new(
            ...
            probe_setting.probe_target.open_tcp_port,
        ),
    );
...
pub(crate) fn build_udp_probe_packet(probe_setting: &ProbeSetting) -> Vec<u8> {
    ...
    match probe_setting.src_ip {
        ...
        IpAddr::V6(src_ipv6) => match probe_setting.probe_target.ip_addr {
            ...
            IpAddr::V6(dst_ipv6) => {
                let ipv6_packet_builder =
                    Ipv6PacketBuilder::new(src_ipv6, dst_ipv6, IpNextLevelProtocol::Udp);
                packet_builder.set_ipv6(ipv6_packet_builder);
            }
        },
    }
...
pub(crate) fn build_icmp_probe_packet(
    probe_setting: &ProbeSetting,
    probe_type: ProbeType,
) -> Vec<u8> {
    let mut packet_builder = PacketBuilder::new();
    ...
    match probe_setting.src_ip {
                ...
                match probe_type {
                    ...
                    ProbeType::IcmpTimestampProbe => {
                        icmp_packet_builder.icmp_type =
                            xenet::packet::icmp::IcmpType::TimestampRequest;
                    }
                    ProbeType::IcmpAddressMaskProbe => {
                        icmp_packet_builder.icmp_type =
                            xenet::packet::icmp::IcmpType::AddressMaskRequest;
                    }
                    ProbeType::IcmpInformationProbe => {
                        icmp_packet_builder.icmp_type =
                            xenet::packet::icmp::IcmpType::InformationRequest;
                    }
                    _ => {
                        icmp_packet_builder.icmp_type = xenet::packet::icmp::IcmpType::EchoRequest;
                    }
        ...
        IpAddr::V6(src_ipv6) => match probe_setting.probe_target.ip_addr {
            ...
            IpAddr::V6(dst_ipv6) => {
                ...
                match probe_type {
                    ProbeType::IcmpEchoProbe => {
                        icmpv6_packet_builder.icmpv6_type =
                            xenet::packet::icmpv6::Icmpv6Type::EchoRequest;
                    }
                    _ => {
                        icmpv6_packet_builder.icmpv6_type =
                            xenet::packet::icmpv6::Icmpv6Type::EchoRequest;
