...
pub(crate) fn receive_packets(
    ...
    stop: &Arc<Mutex<bool>>,
    probe_status: &Arc<Mutex<ProbeStatus>>,
) {
    ...
    if let Some(tcp_packet) = tcp_packet {
            ...
            == pnet_packet::tcp::TcpFlags::RST | pnet_packet::tcp::TcpFlags::ACK
        {
            ...
            == pnet_packet::tcp::TcpFlags::SYN
                ...
                | pnet_packet::tcp::TcpFlags::ECE
        {
            let tcp_header_result: Option<TcpHeaderResult> =
                probe_result.lock().unwrap().tcp_header_result.clone();
    ...
    if let Some(icmp) = icmp_packet {
        match icmp.get_icmp_type() {
            ...
            IcmpTypes::DestinationUnreachable => {
                let icmp_unreach_packet =
                    destination_unreachable::DestinationUnreachablePacket::new(packet.payload())
                        .unwrap();
                let org_ip_packet = pnet_packet::ipv4::Ipv4Packet::new(
                    &icmp_unreach_packet.payload()[..IPV4_HEADER_LEN],
                )
                .unwrap();
                let org_udp_packet = pnet_packet::udp::UdpPacket::new(
                    &icmp_unreach_packet.payload()[IPV4_HEADER_LEN..],
                )
                .unwrap();
                ...
                let org_data_resault = IcmpUnreachableOriginalDataResult {
                    ...
                    udp_header_length: (icmp_unreach_packet.payload().len()
                        - IPV4_HEADER_LEN
                        - org_udp_packet.payload().len())
                        as u16,
                    ...
                    ip_df: if org_ip_packet.get_flags() >= 2 {
                        true
                    } else {
                        false
                    },
    ...
    if let Some(icmp) = icmp_packet {
        match icmp.get_icmpv6_type() {
            ...
            Icmpv6Types::DestinationUnreachable => {
                let org_ip_packet =
                    pnet_packet::ipv6::Ipv6Packet::new(&packet.payload()[..IPV6_HEADER_LEN])
                        .unwrap();
                let org_udp_packet =
                    pnet_packet::udp::UdpPacket::new(&packet.payload()[IPV6_HEADER_LEN..]).unwrap();
                let ip_result: IcmpUnreachableIpResult = IcmpUnreachableIpResult {
                    icmp_unreachable_reply: true,
                ...
                let org_data_resault = IcmpUnreachableOriginalDataResult {
                    udp_checksum: org_udp_packet.get_checksum(),
                    udp_header_length: (packet.payload().len()
                        - IPV4_HEADER_LEN
                        - org_udp_packet.payload().len())
                        as u16,
                    udp_payload_length: org_udp_packet.payload().len() as u16,
...
fn handle_udp_packet(
    ...
    _probe_setting: &ProbeSetting,
    _probe_result: &Arc<Mutex<ProbeResult>>,
) {
}
