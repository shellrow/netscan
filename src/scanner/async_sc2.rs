...
#[derive(Clone, Debug)]
pub struct AsyncSocket2 {
    inner: Arc<Async<Socket>>,
}

impl AsyncSocket2 {
    pub fn new(addr: IpAddr, protocol: Protocol) -> io::Result<AsyncSocket2> {
...
pub struct AsyncHostScanner2 {
    ...
    /// Async Socket
    pub socket: AsyncSocket2,
}

impl AsyncHostScanner2 {
    pub fn new(src_ip: IpAddr) -> Result<AsyncHostScanner2, String> {
        Ok(
            AsyncHostScanner2 {
                src_ip: src_ip,
                dst_ips: vec![],
                ...
                socket: {
                    match src_ip {
                        IpAddr::V4(_) => AsyncSocket2::new(src_ip, Protocol::ICMPV4).unwrap(),
                        IpAddr::V6(_) => AsyncSocket2::new(src_ip, Protocol::ICMPV6).unwrap(),
                    }
                },
            }
        )
...
impl AsyncPortScanner2 {
    pub fn new(src_ip: IpAddr) -> Result<AsyncPortScanner2, String> {
        Ok(
            AsyncPortScanner2 {
                src_ip: src_ip,
                ...
                socket: {
                    match AsyncSocket2::new(src_ip, Protocol::TCP) {
                        Ok(socket) => socket,
...
async fn scan_ports(scanner: AsyncPortScanner2) -> (Vec<PortInfo>, ScanStatus) {
