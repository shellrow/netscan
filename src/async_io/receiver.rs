    ...
    if let Some(tcp_packet) = tcp_packet {
        let host_info: HostInfo = HostInfo {
            ...
            host_name: scan_setting
                .ip_map
                .get(&IpAddr::V4(packet.get_source()))
...
fn tcp_handler_v6(
    ...
    if let Some(tcp_packet) = tcp_packet {
        let host_info: HostInfo = HostInfo {
            ...
            host_name: scan_setting
                .ip_map
                .get(&IpAddr::V6(packet.get_source()))
                .unwrap_or(&String::new())
                .to_string(),
...
fn icmp_handler_v4(
    ...
    if let Some(_icmp) = icmp_packet {
        if scan_setting
            .ip_map
            .contains_key(&IpAddr::V4(packet.get_source()))
            && !scan_result
                .lock()
                .unwrap()
                .ip_set
...
fn icmp_handler_v6(
    ...
    if let Some(_icmp) = icmp_packet {
        if scan_setting
            .ip_map
            .contains_key(&IpAddr::V6(packet.get_source()))
            && !scan_result
                .lock()
                .unwrap()
                .ip_set
...
fn handle_udp_packet(
    ...
    _scan_result: &Arc<Mutex<ScanResult>>,
) {
}
